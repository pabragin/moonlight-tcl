...
public class MediaCodecHelper {
    ...
    // Human readable summary of the low-latency options the decoder kept in its input format,
    ...
    private static boolean isMediaTekDecoder(String decoderName) {
        if (decoderName == null) {
            return false;
        }
        String name = decoderName.toLowerCase(java.util.Locale.ROOT);
        return name.startsWith("omx.mtk") || name.startsWith("c2.mtk");
    }
    ...
    public static boolean setDecoderLowLatencyOptions(MediaFormat videoFormat, MediaCodecInfo decoderInfo, boolean ultraLowLatency, int tryNumber) {
        ...
        if (tryNumber < 3) {
            if (MediaCodecHelper.decoderSupportsMaxOperatingRate(decoderInfo.getName())) {
                ...
                if (ultraLowLatency && isMediaTekDecoder(decoderInfo.getName())) {
                    videoFormat.setInteger(MediaFormat.KEY_OPERATING_RATE, Short.MAX_VALUE);
                }
            }
        }
